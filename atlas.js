(() => {
  ...
  const BASE_CATEGORIES = [
    ...
    ["creature", "◆", "Criatura / Bestiario"], ["npc", "●", "Personaje / NPC"], ["merchant", "⚖", "Comerciante"], ["secret", "✦", "Secreto"],
  ...
  const els = {
    ...
    atlasBattleDialog: $("#atlasBattleDialog"), atlasBattleForm: $("#atlasBattleForm"), atlasBattleMarkerList: $("#atlasBattleMarkerList"), atlasBattleEnd: $("#atlasBattleEnd"),
    playerWaiting: $("#playerWaiting"), playerAtlas: $("#playerAtlas"), playerModeBadge: $("#playerModeBadge"), playerBreadcrumbs: $("#playerBreadcrumbs"), playerMapViewport: $("#playerMapViewport"), playerMapStage: $("#playerMapStage"), playerBattlePanel: $("#playerBattlePanel"), playerBattleCards: $("#playerBattleCards"), playerMerchantPanel: $("#playerMerchantPanel"), leavePlayerMode: $("#leavePlayerMode"), playerZoomOut: $("#playerZoomOut"), playerZoomReset: $("#playerZoomReset"), playerZoomIn: $("#playerZoomIn")
  };
  ...
  function atlas() {
    ...
    if (!campaign.atlas.battle || typeof campaign.atlas.battle !== "object") campaign.atlas.battle = { active: false, sceneId: "", markerIds: [] };
    campaign.atlas.battle.sceneId = String(campaign.atlas.battle.sceneId || "");
    if (!Array.isArray(campaign.atlas.battle.markerIds)) campaign.atlas.battle.markerIds = [];
    campaign.atlas.battle.markerIds = [...new Set(campaign.atlas.battle.markerIds.map(String))];
    ...
    campaign.atlas.scenes.forEach(scene => {
      ...
      scene.worldRefType = ["creature","character","organisation"].includes(String(scene.worldRefType || "")) ? String(scene.worldRefType) : "";
      scene.worldRefId = String(scene.worldRefId || "");
      scene.markers.forEach(marker => {
        ...
        marker.diaryRefs = Array.isArray(marker.diaryRefs) ? marker.diaryRefs.map(ref => ({ missionId: String(ref?.missionId || ""), milestoneId: String(ref?.milestoneId || "") })).filter(ref => ref.missionId) : [];
  ...
  function worldReferenceItems(type) { return window.ForjaWorld?.referenceItems?.(type) || []; }
  function worldReferenceLabel(type, id) { return window.ForjaWorld?.referenceLabel?.(type, id) || ""; }
  function worldReferenceValid(type, id) { return Boolean(type && id && worldReferenceLabel(type, id)); }
  function fillWorldReferenceSelect(typeSelect, idSelect, type = "", id = "") {
    if (!typeSelect || !idSelect) return;
    typeSelect.value = ["creature","character","organisation"].includes(type) ? type : "";
    const items = worldReferenceItems(typeSelect.value);
  ...
  function renderDiaryReferencePicker(selectedRefs = []) {
    const host = els.atlasMarkerDiaryRefs; if (!host) return;
    host.replaceChildren();
    const selected = new Set((selectedRefs || []).map(ref => `${ref.missionId}::${ref.milestoneId || ""}`));
    const groups = window.ForjaDiary?.referenceTree?.() || [];
    if (!groups.length) { host.innerHTML = '<p class="panel__hint">Todavía no hay misiones en Diario.</p>'; if (els.atlasMarkerDiarySummary) els.atlasMarkerDiarySummary.textContent = 'Sin misiones asociadas'; return; }
    let count = selected.size;
    groups.forEach(group => {
      const details = document.createElement('details'); details.className = 'atlas-diary-ref-group';
      const summary = document.createElement('summary'); summary.innerHTML = `<span>${app.escapeHtml(group.name)}</span><small>${group.missions.length}</small>`; details.append(summary);
      const body = document.createElement('div');
      group.missions.forEach(mission => {
        const row = document.createElement('div'); row.className = 'atlas-diary-ref-mission';
        const label = document.createElement('label'); label.innerHTML = `<input type="checkbox" data-diary-ref value="${app.escapeHtml(mission.id)}::" ${selected.has(`${mission.id}::`) ? 'checked' : ''}><span>${mission.type === 'main' ? '★' : mission.type === 'task' ? '☑' : '◆'} <strong>${app.escapeHtml(mission.name)}</strong></span>`; row.append(label);
        if (mission.milestones?.length) { const hitos=document.createElement('details');hitos.className='atlas-diary-ref-milestones';hitos.innerHTML=`<summary>Hitos <small>${mission.milestones.length}</small></summary><div>${mission.milestones.map(h=>`<label><input type="checkbox" data-diary-ref value="${app.escapeHtml(mission.id)}::${app.escapeHtml(h.id)}" ${selected.has(`${mission.id}::${h.id}`)?'checked':''}><span>◇ ${app.escapeHtml(h.name)}</span></label>`).join('')}</div>`;row.append(hitos); }
        body.append(row);
      });
      details.append(body); host.append(details);
    });
    const update = () => { count = host.querySelectorAll('input[data-diary-ref]:checked').length; if (els.atlasMarkerDiarySummary) els.atlasMarkerDiarySummary.textContent = count ? `${count} referencia${count===1?'':'s'}` : 'Sin misiones asociadas'; };
    host.querySelectorAll('input[data-diary-ref]').forEach(input => input.addEventListener('change', update)); update();
  }
  function collectDiaryReferencePicker() {
    if (!els.atlasMarkerDiaryRefs) return [];
    return [...els.atlasMarkerDiaryRefs.querySelectorAll('input[data-diary-ref]:checked')].map(input => { const [missionId,milestoneId] = String(input.value || '').split('::'); return { missionId, milestoneId: milestoneId || '' }; }).filter(ref => ref.missionId);
  }

  function openDiaryPanel(refs = []) {
    const clean = (refs || []).filter(ref => window.ForjaDiary?.missionById?.(ref.missionId));
    if (!clean.length || !els.atlasDmSheet || !els.atlasDmDetailHost) return false;
    if (dmSheetMode === 'notebook' && els.detailPanel?.classList.contains('detail--atlas-dm')) { els.detailPanel.classList.remove('detail--atlas-dm'); els.detailPanelPlaceholder?.after(els.detailPanel); }
    if (dmSheetMode === 'internal') parkInternalSheetForm();
    activeWorldPanel = null; activeDiaryRefs = clean.map(ref => ({...ref})); worldInstanceEditMode = false; dmSheetMode = 'diary'; dmSheetOpen = true; els.atlasDmSheet.hidden = false;
    if (els.atlasDmSheetTitle) els.atlasDmSheetTitle.textContent = '📖 Diario';
    if (els.atlasDmSheetSubtitle) els.atlasDmSheetSubtitle.textContent = clean.length === 1 ? 'Misión / hito asociado al marcador' : `${clean.length} referencias asociadas`;
    if (els.atlasDmSheetEdit) els.atlasDmSheetEdit.hidden = true;
    const picker = els.atlasDmEntryPicker?.closest('.atlas-dm-sheet__picker'); if (picker) picker.hidden = true;
    els.atlasDmDetailHost.innerHTML = window.ForjaDiary?.renderAtlasPanel?.(clean) || ''; window.ForjaDiary?.bindAtlasPanel?.(els.atlasDmDetailHost, clean);
    requestAnimationFrame(() => els.atlasDmDetailHost.scrollTo?.({ top: 0, behavior: 'instant' })); return true;
  }
  function refreshDiaryPanel() {
    if (dmSheetMode !== 'diary' || !activeDiaryRefs.length) return; openDiaryPanel(activeDiaryRefs);
  }
  ...
  function worldReferenceData(type, id) {
    if (type === "creature") return window.ForjaWorld?.getCreature?.(id) || null;
    if (type === "character") return window.ForjaWorld?.getCharacter?.(id) || null;
    if (type === "organisation") return window.ForjaWorld?.getOrganisation?.(id) || null;
    return null;
  }

  function worldPanelSection(title, value) {
    if (!String(value || "").trim()) return "";
    return `<section class="atlas-world-sheet__section"><h4>${app.escapeHtml(title)}</h4><p>${app.escapeHtml(String(value))}</p></section>`;
  }

  function worldPersonPanelHtml(item, type) {
    const labels = { hp: "PG", maxhp: "PG máx.", ac: "CA", speed: "Vel.", initiative: "Inic.", proficiency: "Comp.", str: "FUE", dex: "DES", con: "CON", int: "INT", wis: "SAB", cha: "CAR" };
    const stats = ["hp","maxhp","ac","speed","initiative","proficiency","str","dex","con","int","wis","cha"].filter(key => item?.stats?.[key] !== "" && item?.stats?.[key] !== null && item?.stats?.[key] !== undefined && Number.isFinite(Number(item.stats[key])));
    const sections = [
      worldPanelSection("Modificadores", item.modifiers), worldPanelSection("Habilidades", item.abilities), worldPanelSection("Estilo de combate", item.combatStyle),
      worldPanelSection("Si los PJ no atacan", item.nonAggression), worldPanelSection("Acciones", item.actions), worldPanelSection("Reacciones", item.reactions),
      type === "character" ? worldPanelSection("Notas DM", item.notes) : ""
    ].join("");
    const loot = type === "creature" && Array.isArray(item.loot) && item.loot.length
      ? `<section class="atlas-world-sheet__section"><h4>Botín / recompensas</h4><p>${item.loot.map(part => `${app.escapeHtml(part.name)} ×${Math.max(1,Number(part.quantity)||1)}${part.notes ? ` · ${app.escapeHtml(part.notes)}` : ""}`).join("<br>")}</p></section>` : "";
    const communication = type === "creature" && String(item.communication || "").trim() ? worldPanelSection("Cómo se comunica", item.communication) : "";
    const personality = (type === "character" || item.personality?.fullEnabled) ? atlasPersonalityViewHtml(item.personality) : "";
    return `${communication}${personality}${stats.length ? `<div class="atlas-world-sheet__stats">${stats.map(key => `<div class="atlas-world-sheet__stat"><small>${labels[key]}</small><strong>${app.escapeHtml(String(item.stats[key]))}</strong></div>`).join("")}</div>` : ""}${sections}${loot}`;
  }


  const WORLD_CREATURE_TEXT_FIELDS = ["description","communication","modifiers","abilities","combatStyle","nonAggression","actions","reactions"];
  ...
  function worldInstanceEditorHtml(base, marker) {
    ...
    const textLabels = { description: "Descripción", communication: "Cómo se comunica", modifiers: "Modificadores", abilities: "Habilidades", combatStyle: "Estilo de combate", nonAggression: "Si los PJ no atacan", actions: "Acciones", reactions: "Reacciones" };
  ...
  function renderWorldReferencePanel() {
    ...
    if (worldInstanceEditMode && context.type === "creature" && sourceMarker) {
      ...
      body = `<div class="atlas-world-sheet atlas-world-sheet--${context.type}"><div class="atlas-world-sheet__hero"><div><h3>${app.escapeHtml(displayName)}</h3><p class="atlas-world-sheet__meta">${app.escapeHtml([displayItem.type,displayItem.tags].filter(Boolean).join(" · ") || (context.type === "creature" ? "Criatura" : "Personaje"))}${local ? " · ajustes locales" : ""}</p>${displayItem.description ? `<p class="atlas-world-sheet__description">${app.escapeHtml(displayItem.description)}</p>` : ""}</div><div class="atlas-world-sheet__image" data-atlas-world-image><span>${context.type === "creature" ? "◆" : "●"}</span></div></div>${worldPersonPanelHtml(displayItem,context.type)}</div>`;
    }
  ...
  function battleState() {
    const data = atlas();
    if (!data.battle || typeof data.battle !== "object") data.battle = { active: false, sceneId: "", markerIds: [] };
    const scene = sceneById(data.battle.sceneId);
    if (!scene) { data.battle = { active: false, sceneId: "", markerIds: [] }; return data.battle; }
    const valid = new Set(scene.markers.map(marker => marker.id));
    data.battle.markerIds = [...new Set((data.battle.markerIds || []).filter(id => valid.has(id)))];
    if (!data.battle.markerIds.length) data.battle.active = false;
    return data.battle;
  }

  function battleMarkerPresentation(marker, { publicName = false } = {}) {
    if (!marker) return null;
    let source = null, sourceLabel = "Marcador";
    if (worldReferenceValid(marker.worldRefType, marker.worldRefId)) {
      source = worldReferenceData(marker.worldRefType, marker.worldRefId);
      sourceLabel = marker.worldRefType === "creature" ? "Criatura de Mundo" : marker.worldRefType === "character" ? "Personaje de Mundo" : "Organización de Mundo";
    } else if (marker.sheetMode === "internal") {
      source = markerSheetById(marker.internalSheetId);
      sourceLabel = "Ficha interna del Atlas";
    } else if (marker.sheetMode === "notebook") {
      source = entryById(marker.primaryEntryId) || marker.relatedEntryIds?.map(entryById).find(Boolean) || null;
      sourceLabel = "Ficha del Cuaderno";
    }
    const rawName = publicName && marker.category === "npc" ? (marker.alias || marker.name || source?.name) : (marker.name || source?.name || marker.alias);
    return { markerId: marker.id, name: String(rawName || "Enemigo"), imageId: String(source?.imageId || ""), sourceLabel };
  }

  function battleCardsForScene(scene, { publicNames = false } = {}) {
    const battle = battleState();
    if (!battle.active || !scene || battle.sceneId !== scene.id) return [];
    return battle.markerIds.map(id => battleMarkerPresentation(scene.markers.find(marker => marker.id === id), { publicName: publicNames })).filter(Boolean);
  }

  function renderBattleCardHost(host, cards, classPrefix) {
    if (!host) return;
    host.replaceChildren();
    cards.forEach(card => {
      const figure=document.createElement("figure"); figure.className=`${classPrefix}-card`;
      const image=document.createElement("div"); image.className=`${classPrefix}-card__image`; image.textContent="◆";
      ...
      if (card.imageId) imageUrl(card.imageId).then(url=>{if(!url||!image.isConnected)return;image.replaceChildren();const img=document.createElement("img");img.src=url;img.alt=card.name;image.append(img);}).catch(()=>{});
    });
  }

  function renderBattlePanel(scene = currentScene()) {
    if (!els.atlasBattlePanel || !els.atlasLayout) return;
    const cards = battleCardsForScene(scene);
    const visible = cards.length > 0;
    els.atlasBattlePanel.hidden = !visible;
    els.atlasLayout.classList.toggle("is-battle-open", visible);
    if (els.atlasBattleBtn) els.atlasBattleBtn.textContent = visible ? `⚔ Batalla · ${cards.length}` : "⚔ Batalla";
    if (visible) renderBattleCardHost(els.atlasBattleCards, cards, "atlas-battle");
    requestAnimationFrame(() => { if (scene?.imageId && state().view === "atlas") { fitAtlasStage(scene); resizeFogCanvas(scene); renderSceneOverlays(scene); } });
  }

  function renderBattleMarkerList(scene = currentScene()) {
    if (!els.atlasBattleMarkerList || !scene) return;
    const battle = battleState();
    const selected = new Set(battle.sceneId === scene.id ? battle.markerIds : []);
    const markers = scene.markers.filter(markerIsUnlocked);
    els.atlasBattleMarkerList.replaceChildren();
    if (!markers.length) { els.atlasBattleMarkerList.innerHTML = '<p class="dialog-intro">Esta escena no tiene marcadores disponibles.</p>'; return; }
    markers.forEach(marker => {
      const data = battleMarkerPresentation(marker);
      const label=document.createElement("label"); label.className="atlas-battle-choice";
      const input=document.createElement("input"); input.type="checkbox"; input.value=marker.id; input.checked=selected.has(marker.id);
      const thumb=document.createElement("span"); thumb.className="atlas-battle-choice__thumb"; thumb.textContent=categoryMeta(marker.category,marker.icon).icon;
      const text=document.createElement("span"); text.className="atlas-battle-choice__text"; const strong=document.createElement("strong"); strong.textContent=data?.name || marker.name || "Marcador"; const small=document.createElement("small"); small.textContent=data?.sourceLabel || categoryMeta(marker.category,marker.icon).label; text.append(strong,small); label.append(input,thumb,text); els.atlasBattleMarkerList.append(label);
      if (data?.imageId) imageUrl(data.imageId).then(url=>{if(!url||!thumb.isConnected)return;thumb.replaceChildren();const img=document.createElement("img");img.src=url;img.alt="";thumb.append(img);}).catch(()=>{});
    });
  }

  function openBattleDialog() {
    const scene=currentScene(); if(!scene||!els.atlasBattleDialog)return;
    renderBattleMarkerList(scene);
    const battle=battleState(); if(els.atlasBattleEnd) els.atlasBattleEnd.hidden=!(battle.active && battle.sceneId===scene.id);
    els.atlasBattleDialog.showModal();
  }

  function endBattle() {
    const data=atlas(); data.battle={active:false,sceneId:"",markerIds:[]};
    els.atlasBattleDialog?.close?.(); save({publish:true,render:true,immediate:true});
  }

  function saveBattleDialog(event) {
    event.preventDefault(); const scene=currentScene(); if(!scene)return;
    const ids=$$('input[type="checkbox"]:checked',els.atlasBattleMarkerList).map(input=>input.value).filter(id=>scene.markers.some(marker=>marker.id===id));
    if(!ids.length){ alert("Selecciona al menos un marcador para la batalla."); return; }
    atlas().battle={active:true,sceneId:scene.id,markerIds:ids}; els.atlasBattleDialog?.close?.(); save({publish:true,render:true,immediate:true});
  }
  ...
  function defaultMarkerSheet(category, name, markerId = "") {
    return {
      id: app.uid(), markerId, category, kind: markerSheetKind(category) || "creature", name: String(name || (category === "npc" ? "" : "Ficha")).slice(0, 100), imageId: "", description: "", communication: "", personality: app.blankPersonality?.() || {traits:[],fullEnabled:false},
  ...
  function normaliseMarkerSheet(raw = {}) {
    ...
    return {
      ...
      name: String(raw.name || base.name).slice(0, 100), imageId: String(raw.imageId || ""), description: String(raw.description || "").slice(0, 8000), communication: String(raw.communication || "").slice(0,3000), personality: app.normalisePersonality?.(raw.personality) || raw.personality || base.personality, stats,
  ...
  function markerHasSheet(marker) {
    ...
    if (worldReferenceValid(marker.worldRefType, marker.worldRefId)) return true;
  ...
  async function syncLanProjection(snapshot = publicSnapshot()) {
    ...
    try {
      ...
      const imageIds = [...new Set([
        ...snapshot.scenes.map(scene => scene.imageId).filter(Boolean),
        ...(snapshot.battle?.cards || []).map(card => card.imageId).filter(Boolean)
      ])];
  ...
  function renderAtlas() {
    ...
    renderSceneTree();
    renderCategories();
    if (dmSheetOpen && dmSheetMode === "notebook") {
  ...
  function markerVisual(marker) {
    ...
    const tones = {
      ...
      danger: "danger", combat: "danger", creature: "danger", portal: "portal"
    };
  ...
  function stopMarkerDrag(event) {
    ...
    if (moved) save({ publish: true });
      ...
      const hasDiary = marker.diaryRefs?.length; const openSheetAfterNavigation = markerHasSheet(marker);
      setScene(marker.targetSceneId, { sourceMarker: marker });
      if (hasDiary) setTimeout(() => openDiaryPanel(marker.diaryRefs), 0);
      else if (openSheetAfterNavigation) setTimeout(() => openMarkerSheet(marker), 0);
    }
    else if (marker.diaryRefs?.length) openDiaryPanel(marker.diaryRefs);
  ...
  function openSceneDialog(sceneId = "") {
    ...
    els.atlasDeleteScene.hidden = !editing || atlas().scenes.length === 1; if (els.atlasDuplicateScene) els.atlasDuplicateScene.hidden = !editing;
  ...
  function renderMarkerSheetMode(marker = markerDialogScene()?.markers.find(item => item.id === editingMarkerId) || null) {
    ...
    const hasWorldReference = worldReferenceValid(String(els.atlasMarkerWorldRefType?.value || ""), String(els.atlasMarkerWorldRefId?.value || ""));
    els.atlasMarkerSheetModeWrap.hidden = !supported || hasWorldReference;
    ...
    const mode = supported && !hasWorldReference ? currentMarkerSheetMode() : "";
  ...
  function openMarkerSheet(marker) {
    if (!marker) return;
    if (worldReferenceValid(marker.worldRefType, marker.worldRefId)) { els.atlasMarkerDialog?.close?.(); openWorldReferencePanel(marker.worldRefType, marker.worldRefId, marker); return; }
    if (marker.sheetMode === "internal") {
  ...
  const ATLAS_PERSONALITY_TRAITS = app.personalityTraits || [];
  const ATLAS_PERSONALITY_FIELDS = [["speech","Cómo habla"],["conduct","Qué hace / conducta habitual"],["appearance","Apariencia"],["wants","Qué quiere en realidad"],["past","Qué ha vivido antes"],["origin","De dónde viene"],["reactionToPlayers","Por qué reacciona así ante los personajes"],["contradictions","Contradicciones"],["emotionalAnchor","Ancla emocional"],["entrance","Entrada en escena"],["accent","Acento / pronunciación"],["culturalLevel","Nivel cultural"],["expressions","Expresiones habituales"],["bodyLanguage","Expresión corporal"],["tics","Tics"],["obsessions","Obsesiones"]];
  const ATLAS_VOICE_LABELS={high:"Agudo",neutral:"Neutral",low:"Grave"};
  function fillAtlasPersonalityEditor(root, raw) { if(!root)return;const p=app.normalisePersonality?.(raw)||raw||{traits:[]};const host=root.querySelector('[data-personality-traits]');if(host)host.innerHTML=ATLAS_PERSONALITY_TRAITS.map(([value,label])=>`<label class="personality-trait"><input type="checkbox" value="${app.escapeHtml(value)}" ${(p.traits||[]).includes(value)?'checked':''}><span>${app.escapeHtml(label)}</span></label>`).join('');root.querySelectorAll('[data-personality-field]').forEach(control=>{const key=control.dataset.personalityField;control.value=key==='voiceRange'?(p.voiceRange||''):(p[key]||'');}); }
  function readAtlasPersonalityEditor(root, raw) {const p=app.normalisePersonality?.(raw)||raw||{traits:[]};if(!root)return p;p.traits=[...root.querySelectorAll('[data-personality-traits] input:checked')].map(input=>input.value).slice(0,20);root.querySelectorAll('[data-personality-field]').forEach(control=>{p[control.dataset.personalityField]=control.value;});return app.normalisePersonality?.(p)||p;}
  function atlasPersonalityViewHtml(raw){const p=app.normalisePersonality?.(raw)||raw||{traits:[]};if(!app.personalityHasContent?.(p))return'';const chips=(p.traits||[]).map(value=>`<span class="personality-chip">${app.escapeHtml(ATLAS_PERSONALITY_TRAITS.find(item=>item[0]===value)?.[1]||value)}</span>`).join('');const fields=ATLAS_PERSONALITY_FIELDS.map(([key,label])=>String(p[key]||'').trim()?`<div class="personality-read-field"><strong>${app.escapeHtml(label)}</strong><p>${app.escapeHtml(p[key])}</p></div>`:'').join('');const voice=p.voiceRange?`<div class="personality-read-field"><strong>Rango vocal</strong><p>${app.escapeHtml(ATLAS_VOICE_LABELS[p.voiceRange]||p.voiceRange)}</p></div>`:'';return `<details class="personality-panel personality-read"><summary><span>🎭 Personalidad e interpretación</span><small>Desplegar</small></summary><div class="personality-panel__body">${chips?`<div class="personality-chip-row">${chips}</div>`:''}<div class="personality-read-grid">${fields}${voice}</div></div></details>`;}
  ...
  function refreshInternalSheetViewVisibility() {
    ...
    if (els.atlasPersonalityDetails && !els.atlasPersonalityDetails.hidden) {
      const hasPersonality = app.personalityHasContent?.(sheet.personality);
      const hasCommunication = String(sheet.communication || "").trim();
      if (!hasPersonality && !hasCommunication) els.atlasPersonalityDetails.classList.add("view-hide");
    }
  ...
  function loadInternalSheetForm() {
    ...
    if (els.atlasInternalSheetForm) els.atlasInternalSheetForm.dataset.sheetCategory = sheet.category || "";
    const category = categoryMeta(sheet.category);
    ...
    sheet.personality = app.normalisePersonality?.(sheet.personality) || sheet.personality || {traits:[],fullEnabled:false};
    if (els.atlasPersonalityDetails) {
      const isPerson = sheet.category === "npc" || sheet.category === "merchant";
      const isCreaturePersonality = sheet.category === "creature";
      els.atlasPersonalityDetails.hidden = !(isPerson || isCreaturePersonality);
      els.atlasPersonalityDetails.open = false;
      if (els.atlasPersonalityCommunicationWrap) els.atlasPersonalityCommunicationWrap.hidden = !isCreaturePersonality;
      if (els.atlasPersonalityFullToggleWrap) els.atlasPersonalityFullToggleWrap.hidden = !isCreaturePersonality;
      if (els.atlasPersonalityCommunication) els.atlasPersonalityCommunication.value = sheet.communication || "";
      if (els.atlasPersonalityFullToggle) els.atlasPersonalityFullToggle.checked = isPerson ? true : Boolean(sheet.personality.fullEnabled);
      if (els.atlasPersonalityFullFields) els.atlasPersonalityFullFields.hidden = isCreaturePersonality && !sheet.personality.fullEnabled;
      fillAtlasPersonalityEditor(els.atlasPersonalityFullFields, sheet.personality);
    }
  ...
  function collectInternalSheetForm() {
    ...
    if (sheet.category === "npc" || sheet.category === "merchant" || sheet.category === "creature") {
      sheet.communication = sheet.category === "creature" ? String(els.atlasPersonalityCommunication?.value || "").slice(0,3000) : "";
      sheet.personality = readAtlasPersonalityEditor(els.atlasPersonalityFullFields, sheet.personality);
      sheet.personality.fullEnabled = sheet.category === "creature" ? Boolean(els.atlasPersonalityFullToggle?.checked) : true;
    }
  ...
  function syncDiaryReverseRefsForMarker(marker) {
    if (!marker) return;
    const d = app.getState()?.diary; if (!d?.missions) return;
    const refs = Array.isArray(marker.diaryRefs) ? marker.diaryRefs : [];
    const selectedMissionIds = new Set(refs.map(ref => ref.missionId).filter(Boolean));
    d.missions.forEach(mission => {
      mission.atlasMarkerIds = Array.isArray(mission.atlasMarkerIds) ? mission.atlasMarkerIds.filter(Boolean) : [];
      const has = mission.atlasMarkerIds.includes(marker.id);
      if (selectedMissionIds.has(mission.id) && !has) mission.atlasMarkerIds.push(marker.id);
      if (!selectedMissionIds.has(mission.id) && has) mission.atlasMarkerIds = mission.atlasMarkerIds.filter(id => id !== marker.id);
      (mission.milestones || []).forEach(h => {
        const linked = refs.some(ref => ref.missionId === mission.id && ref.milestoneId === h.id);
        if (linked) h.atlasMarkerId = marker.id;
        else if (h.atlasMarkerId === marker.id) h.atlasMarkerId = "";
      });
    });
  }

  function removeDiaryReverseRefsForMarker(markerId) {
    const d = app.getState()?.diary; if (!d?.missions || !markerId) return;
    d.missions.forEach(mission => {
      mission.atlasMarkerIds = (mission.atlasMarkerIds || []).filter(id => id !== markerId);
      (mission.milestones || []).forEach(h => { if (h.atlasMarkerId === markerId) h.atlasMarkerId = ""; });
    });
  }
  ...
  function openMarkerDialog(markerId = "", point = null, sceneId = "") {
    ...
    renderDiaryReferencePicker(marker?.diaryRefs || []);
  ...
  function saveMarkerDialog(event) {
    ...
    const hasWorldReference = worldReferenceValid(String(els.atlasMarkerWorldRefType?.value || ""), String(els.atlasMarkerWorldRefId?.value || ""));
    const requestedMode = hasWorldReference ? "" : (supported ? currentMarkerSheetMode() : (relatedEntryIds.length ? "notebook" : ""));
    ...
    const patch = {
      ...
      worldRefType: String(els.atlasMarkerWorldRefType?.value || ""), worldRefId: String(els.atlasMarkerWorldRefId?.value || ""), diaryRefs: collectDiaryReferencePicker(),
    ...
    syncDiaryReverseRefsForMarker(marker);
  ...
  function duplicateMarker() {
    const scene = markerDialogScene(); const source = scene?.markers.find(item => item.id === editingMarkerId); if (!scene || !source) return;
    const copy = app.clone(source); copy.id = app.uid(); copy.name = source.name ? `${source.name} (copia)` : source.name; copy.x = clamp((Number(source.x)||0.5)+.025); copy.y = clamp((Number(source.y)||0.5)+.025); copy.createdAt = app.now(); copy.updatedAt = app.now(); scene.markers.push(copy); syncDiaryReverseRefsForMarker(copy);
    els.atlasMarkerDialog.close(); save({ render: true }); openMarkerDialog(copy.id, null, scene.id);
  }

  function duplicateScene() {
    const source = sceneById(editingSceneId); if (!source) return; const copy = app.clone(source); copy.id = app.uid(); copy.name = `${source.name} (copia)`; copy.parentSceneId = source.parentSceneId; copy.markers = (copy.markers||[]).map(m=>({...m,id:app.uid(),createdAt:app.now(),updatedAt:app.now()})); copy.dmSourceMarkerId = ''; copy.createdAt=app.now(); copy.updatedAt=app.now(); atlas().scenes.push(copy); (copy.markers||[]).forEach(syncDiaryReverseRefsForMarker); els.atlasSceneDialog.close(); save({render:true}); setScene(copy.id); openSceneDialog(copy.id);
  }
  ...
  function deleteMarker() {
    ...
    window.ForjaWorkspace?.pushTrash?.("atlasMarker", markerDmLabel(marker), { sceneId: scene.id, marker: app.clone(marker) });
    clearMarkerSceneLink(marker.id, marker.targetSceneId);
    removeDiaryReverseRefsForMarker(marker.id);
  ...
  function publicSnapshot() {
    ...
    const battleScene = activeBattle.active ? sceneById(activeBattle.sceneId) : null;
    const battleCards = battleScene ? battleCardsForScene(battleScene, { publicNames: true }).map(card => ({ markerId: card.markerId, name: card.name, imageId: card.imageId })) : [];
    const publicBattle = { active: Boolean(activeBattle.active && battleScene && battleCards.length), sceneId: battleScene?.id || "", cards: battleCards };
    return {
      version: 3, campaignId: state().id, campaignName: state().campaignName, updatedAt: Date.now(),
      projectionSceneId: data.projectionSceneId, playerNavigationMode: data.playerNavigationMode, publicMerchantEntryId: data.publicMerchantEntryId, battle: publicBattle,
  ...
  async function renderPlayer() {
    ...
    if (!playerSnapshot?.scenes?.length || !playerSnapshot.projectionSceneId) {
      els.playerWaiting.hidden = false; els.playerAtlas.hidden = true; els.playerAtlas?.classList.remove("has-battle-panel"); if (els.playerBattlePanel) els.playerBattlePanel.hidden = true; return;
    }
    ...
    if (!scene) { els.playerWaiting.hidden = false; els.playerAtlas.hidden = true; els.playerAtlas?.classList.remove("has-battle-panel"); if (els.playerBattlePanel) els.playerBattlePanel.hidden = true; return; }
  ...
  function renderPlayerBattle(scene) {
    if (!els.playerBattlePanel || !els.playerBattleCards || !els.playerAtlas) return;
    const battle = playerSnapshot?.battle;
    const cards = battle?.active && battle.sceneId === scene?.id && Array.isArray(battle.cards) ? battle.cards : [];
    const visible = cards.length > 0;
    els.playerBattlePanel.hidden = !visible;
    els.playerAtlas.classList.toggle("has-battle-panel", visible);
    els.playerBattleCards.replaceChildren();
    if (!visible) return;
    cards.forEach(card => {
      const figure=document.createElement("figure"); figure.className="player-battle-card";
      const image=document.createElement("div"); image.className="player-battle-card__image"; image.textContent="◆";
      const caption=document.createElement("figcaption"); caption.textContent=card.name || "Enemigo";
      figure.append(image,caption); els.playerBattleCards.append(figure);
      if(card.imageId) imageUrl(card.imageId).then(url=>{if(!url||!image.isConnected)return;image.replaceChildren();const img=document.createElement("img");img.src=url;img.alt=card.name||"";image.append(img);}).catch(()=>{});
    });
  }
  ...
  async function createFullBackupBlob() {
    const ids = new Set(profile().campaigns.flatMap(campaign => [
      ...
      ...(campaign.world?.creatures?.map(item => item.imageId).filter(Boolean) || []),
      ...(campaign.world?.characters?.map(item => item.imageId).filter(Boolean) || []),
      ...(campaign.world?.organisations?.map(item => item.imageId).filter(Boolean) || []),
      ...(campaign.world?.creatureCategories?.map(item => item.imageId).filter(Boolean) || [])
    ]));
  ...
  function bind() {
    ...
    els.atlasBattleBtn?.addEventListener("click", openBattleDialog);
    els.atlasBattleEdit?.addEventListener("click", openBattleDialog);
    els.atlasBattleClose?.addEventListener("click", endBattle);
    ...
    els.atlasSceneForm.addEventListener("submit", saveSceneDialog); els.atlasDeleteScene.addEventListener("click", deleteScene); els.atlasDuplicateScene?.addEventListener("click", duplicateScene);
    ...
    els.atlasMarkerForm.addEventListener("submit", saveMarkerDialog); els.atlasDeleteMarker.addEventListener("click", deleteMarker); els.atlasDuplicateMarker?.addEventListener("click", duplicateMarker);
    ...
    els.atlasDmSheetEdit?.addEventListener("click", () => { if (dmSheetMode === "world") toggleWorldInstanceEdit(); else if (dmSheetMode === "internal") toggleInternalSheetEdit(); });
    ...
    els.atlasPersonalityFullToggle?.addEventListener("change", () => { if (els.atlasPersonalityFullFields) els.atlasPersonalityFullFields.hidden = !els.atlasPersonalityFullToggle.checked; });
    ...
    els.atlasBattleForm?.addEventListener("submit", saveBattleDialog);
